...
Copyright (C) 2015-2018 CartoType Ltd.
...
class TVectorObjectStyle
    ...
    /** The interval between strokes of a hachure used to fill polygons; ignored if m_hachure_stroke_width is not positive. */
    float m_hachure_interval = 0;
    ...
    bool m_draw_height = false;
    };

class TVectorObjectGroup
    {
    public:
    int32 m_layer_group = 0;
    uint32 m_priority = 0;
...
class CPositionedLabel
    ...
    public:
    CPositionedLabel() { }
    CPositionedLabel(uint32 aLayer,const TBitmap& aBitmap,const TPoint& aTopLeft,const TPoint& aHotspot):
        m_layer(aLayer),
        m_bitmap(aBitmap),
        m_top_left(aTopLeft),
        m_hotspot(aHotspot)
...
class CVectorTileHelper
    ...
    */
    virtual void OnStartDrawing(const TMapState& /*aMapState*/,const std::vector<TRect>& /*aBackgroundRectArray*/) { }
...
*/
std::unique_ptr<CVectorTileServer> CreateOpenGLESVectorTileServer(CFramework& aFramework);
