...
template<class T> class TPoint2
    ...
    T DistanceFrom(const TPoint2<T>& aOther) const
        {
        TPoint2<T> v { *this };
        v -= aOther;
...
template<class T> class TPoint3
    ...
    void operator-=(const TPoint3<T>& aPoint)  { iX -= aPoint.iX; iY -= aPoint.iY; iZ -= aPoint.iZ; }
    /** Scale a point by a factor. */
    void operator*=(T aFactor) { iX *= aFactor; iY *= aFactor; iZ *= aFactor; }
    ...
    T DistanceFrom(const TPoint3<T>& aOther) const
        {
        TPoint3<T> v { *this };
        v -= aOther;
        return v.VectorLength();
    ...
    static TPoint3<T> Normal(const TPoint3<T>& aA,const TPoint3<T>& aB,const TPoint3<T>& aC)
        {
        TPoint3<T> b { aB }; b -= aA;
        TPoint3<T> c { aC }; c -= aA;
