...
Copyright (C) 2012-2018 CartoType Ltd.
...
/** A TNavigationState synonym for backward compatibility. @deprecated */
constexpr TNavigationState ENoNavigationState = TNavigationState::None;
/** A TNavigationState synonym for backward compatibility. @deprecated */
constexpr TNavigationState ETurnNavigationState = TNavigationState::Turn;
/** A TNavigationState synonym for backward compatibility. @deprecated */
constexpr TNavigationState ETurnRoundNavigationState = TNavigationState::TurnRound;
/** A TNavigationState synonym for backward compatibility. @deprecated */
constexpr TNavigationState ENewRouteNavigationState = TNavigationState::NewRoute;
/** A TNavigationState synonym for backward compatibility. @deprecated */
constexpr TNavigationState EArrivalNavigationState = TNavigationState::Arrival;
/** A TNavigationState synonym for backward compatibility. @deprecated */
constexpr TNavigationState EOffRouteNavigationState = TNavigationState::OffRoute;
/** A TNavigationState synonym for backward compatibility. @deprecated */
constexpr TNavigationState EReRouteNeededNavigationState = TNavigationState::ReRouteNeeded;

/** A flag to make the center of the map follow the user's location. */
constexpr uint32 KFollowFlagLocation = 1;
/** A flag to rotate the map to the user's heading. */
constexpr uint32 KFollowFlagHeading = 2;
/** A flag to set the map to a suitable zoom level for the user's speed. */
constexpr uint32 KFollowFlagZoom = 4;
...
/** A TFollowMode synonym for backward compatibility. @deprecated */
constexpr TFollowMode EFollowModeNone = TFollowMode::None;
/** A TFollowMode synonym for backward compatibility. @deprecated */
constexpr TFollowMode EFollowModeLocation = TFollowMode::Location;
/** A TFollowMode synonym for backward compatibility. @deprecated */
constexpr TFollowMode EFollowModeLocationHeading = TFollowMode::LocationHeading;
/** A TFollowMode synonym for backward compatibility. @deprecated */
constexpr TFollowMode EFollowModeLocationZoom = TFollowMode::LocationZoom;
/** A TFollowMode synonym for backward compatibility. @deprecated */
constexpr TFollowMode EFollowModeLocationHeadingZoom = TFollowMode::LocationHeadingZoom;
...
class TPerspectiveParam
    ...
    bool operator==(const TPerspectiveParam& aP) const
        {
        return std::forward_as_tuple(iPosition,iCoordType,iAutoPosition,iHeightInMeters,iAzimuthDegrees,iAutoAzimuth,iDeclinationDegrees,iRotationDegrees,iFieldOfViewDegrees) == 
               std::forward_as_tuple(aP.iPosition,aP.iCoordType,aP.iAutoPosition,aP.iHeightInMeters,aP.iAzimuthDegrees,aP.iAutoAzimuth,aP.iDeclinationDegrees,aP.iRotationDegrees,aP.iFieldOfViewDegrees);
        }
    bool operator!=(const TPerspectiveParam& aP) const
        {
        return !(*this == aP);
    ...
    bool operator<(const TPerspectiveParam& aP) const
        {
        return std::forward_as_tuple(iPosition,int(iCoordType),iAutoPosition,iHeightInMeters,iAzimuthDegrees,iAutoAzimuth,iDeclinationDegrees,iRotationDegrees,iFieldOfViewDegrees) <
               std::forward_as_tuple(aP.iPosition,int(aP.iCoordType),aP.iAutoPosition,aP.iHeightInMeters,aP.iAzimuthDegrees,aP.iAutoAzimuth,aP.iDeclinationDegrees,aP.iRotationDegrees,aP.iFieldOfViewDegrees);
        }
...
class CFramework: public MNavigatorObserver
    ...
    */
    ...
    bool MapIsWritable(size_t aIndex) const;
    TResult UnloadMapByHandle(uint32 aHandle);
    void EnableMapByHandle(uint32 aHandle,bool aEnable);
    void EnableAllMaps();
    uint32 GetLastMapHandle() const;
    uint32 GetMainMapHandle() const;
    uint32 GetMemoryMapHandle() const;
    ...
    void ForceRedraw();
    void CancelDrawing();
    void UseImageServer(MImageServerHelper* aImageServerHelper,bool aDrawMemoryDataBase = false,int32 aCacheSize = KDefaultImageCacheSize);
    void UseGcImageServer(bool aEnable,int32 aCacheSize = KDefaultImageCacheSize);
    ...
    CBitmap TileBitmap(TResult& aError,int32 aTileSizeInPixels,int32 aZoom,int32 aX,int32 aY,const TTileBitmapParam* aParam = nullptr);
    CBitmap TileBitmap(TResult& aError,int32 aTileSizeInPixels,const CString& aQuadKey,const TTileBitmapParam* aParam = nullptr);
    CBitmap TileBitmap(TResult& aError,int32 aTileWidth,int32 aTileHeight,const TRectFP& aBounds,TCoordType aCoordType,const TTileBitmapParam* aParam = nullptr);
    ...
    // terrain heights
    TResult GetHeights(const TCoordSet& aCoordSet,TCoordType aCoordType,int32* aTerrainHeightArray) const;
    ...
    void SetStyleSheetVariable(const CString& aVariableName,int32 aValue);
    ...
    static constexpr uint64 KRouteIdReservedStart = 0x40000000;

    /** A reserved map object ID for the first route. */
    static constexpr uint64 KFirstRouteId = KRouteIdReservedStart;
    /** A reserved map object ID for the last route. */
    static constexpr uint64 KLastRouteId = KRouteIdReservedStart + KMaxRoutesDisplayed - 1;
    /** A reserved map object ID for the start of the route. */
    static constexpr uint64 KRouteStartId = KLastRouteId + 1;
    /** A reserved map object ID for the end of the route. */
    static constexpr uint64 KRouteEndId = KLastRouteId + 2;
    /** A reserved map object ID for a point object at the current position on the route. */
    static constexpr uint64 KRoutePositionId = KLastRouteId + 3;
    /** A reserved map object ID for a vector object centered on the current position on the route. */
    static constexpr uint64 KRouteVectorId = KLastRouteId + 4;
    /** A reserved map object ID for the first waypoint. */
    static constexpr uint64 KFirstWayPointId = KLastRouteId + 5;
    /** One more than the last ID reserved for route-related map objects. */
    static constexpr uint64 KRouteIdReservedEnd = KFirstWayPointId + KMaxWayPointsDisplayed;
    ...
    // Notifying the framework observers.
    ...
    void StyleChanged() { for (auto p : iFrameworkObservers) p->OnStyleChange(); }
    void LayerChanged() { for (auto p : iFrameworkObservers) p->OnLayerChange(); }
    void NoticeChanged() { for (auto p : iFrameworkObservers) p->OnNoticeChange(); }
...
CString UKGridReferenceFromMapPoint(const TPointFP& aPointInMapMeters,int32 aDigits);
CString UKGridReferenceFromDegrees(const TPointFP& aPointInDegrees,int32 aDigits);
