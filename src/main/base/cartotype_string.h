...
class MString
    ...
    */
    ...
    static int32 Compare(MIter<int32>& aIter1,MIter<int32>& aIter2,TStringMatchMethod aStringMatchMethod) noexcept;
    ...
    int32 Compare(MIter<int32>& aIter,bool aFoldCase) const noexcept;
    TResult Replace(size_t aStart,size_t aEnd,MIter<int32>& aText,size_t aMaxLength);

    // functions taking an MString
    int32 Compare(const MString& aString) const
        {
        auto n = iLength;
        auto m = aString.iLength;
        const uint16* p = Text();
        const uint16* q = aString.Text();
        while (n != 0 && m != 0)
            {
            if (*p < *q)
                return -2;
    ...
    int32 Compare(const MString& aString,bool aFoldCase) const noexcept;
    int32 Compare(const MString& aString,TStringMatchMethod aStringMatchMethod) const noexcept;
    ...
    int32 Compare(const uint16* aText) const
        {
        auto n = iLength;
        const uint16* p = Text();
        const uint16* q = aText;
        while (n-- != 0)
            {
            if (*q == 0)
                return 1;
    ...
    int32 Compare(const uint16* aText,size_t aLength) const
        {
        if (aLength == npos)
    ...
    int32 Compare(const char16_t* aText,size_t aLength = npos) const { return Compare((const uint16*)aText,aLength); }
    int32 Compare(const uint16* aText,size_t aLength,bool aFoldCase) const noexcept;
    ...
    int32 Compare(const char* aText,size_t aLength = npos,bool aFoldCase = false) const noexcept;
    ...
    */
    ...
    bool operator==(const MString& aString) const
        {
        return aString.iLength == iLength && !memcmp(aString.Text(),Text(),iLength * sizeof(uint16));
        }
    ...
    bool operator!=(const uint16* aText) const { return !(*this == aText); }
    /** The inequality operator for comparing MString objects with null-terminated UTF16 strings */
    ...
    static bool FuzzyMatch(MIter<int32>& aIter1,MIter<int32>& aIter2,int32 aMaxDistance) noexcept;
    static bool FuzzyMatch(const char* aText1,const char* aText2,int32 aMaxDistance) noexcept;
    ...
    static void ToInt32(const uint8* aText,size_t aLength,int32& aValue,size_t& aLengthUsed,int32 aBase = 10) noexcept;
    void ToInt32(int32& aValue,size_t& aLengthUsed,int32 aBase = 10) const noexcept;
    void ToInt64(int64& aValue,size_t& aLengthUsed,int32 aBase = 10) const noexcept;
    void ToUint32(uint32& aValue,size_t& aLengthUsed,int32 aBase = 10) const noexcept;
