...
class CStyleSheetData
    ...
    CStyleSheetData(TResult& aError,const uint8* aData,size_t aLength);
    TResult Reload();
    TMemoryInputStream Stream() const { return TMemoryInputStream((const uint8*)iText.data(),iText.length()); }
